...
static int my_open (struct inode *inode,
	struct file *filp)
{
	if (device_open) // if device_open != 0, leave as it is busy
		return -EBUSY;
	...
	size_t length, loff_t *offset)
{
	...
	// copy_to_user is a function contained in the asm library
...
static int __init my_init (void)
{
	...
	//device_open = 0; // device has just been created, it's free
	
	/*
	// buf is a block in memory
	buf = kcalloc (BUF_LEN, sizeof(char), GFP_KERNEL);
...
static void __exit my_cleanup (void)
{
	// free the dynamically allocated array, if it is valid
	/*
	if (buf)
		kfree (buf);
	*/
